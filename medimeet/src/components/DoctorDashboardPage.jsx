...
import { AvailabilitySettings } from "./doctor-dashboard/availability-settings";
import { DoctorEarnings } from "./doctor-dashboard/doctor-earnings";
import DoctorAppointmentsList from "./doctor-dashboard/appointments-list";

export default async function DoctorDashboardPage({ user }) {
    ...
    <Tabs defaultValue="earnings" className="grid grid-cols-1 md:grid-cols-4 gap-6">
      ...
      <div className="md:col-span-3">
        ...
        <TabsContent value="earnings" className="border-none p-0">
          <DoctorEarnings earnings={earningsData.earnings || {}} payouts={payoutsData.payouts || []} />
        </TabsContent>
      </div>
    </Tabs>
  );
